...
//Used only for the design problem
...
//function used for crashing
int crash_ioctl(struct inode *inode, struct file *filp,
	unsigned int compare, unsigned long end)
{
  if(compare == IOCTLCRASH)
    {
      nwrites_to_crash = (int) end;
      return 0;
    }
  else
    return -ENOTTY;
